...
You have been asked to prepare an aggregation that represents the number of
user visits to all microservices, for use by data analysts.
...
There are some problems, however:
  ...
  - The map key, which is a String, should be parseable to Long -
    but it may not be. You must skip any such faulty entries.(())

  - For some keys, UserStats may be null. You must skip any such faulty entries.(())

  - UserStats has a single field, visitCount, of type Optional<Long>.
    A getter for this field is also implemented. This field will never be null;
